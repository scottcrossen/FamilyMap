...
import com.scottcrossen42.familymap.ui.RecyclerView.PersonRecycler.Section;

import java.util.ArrayList;
import java.util.Iterator;
import java.util.List;

public class PersonActivity extends AppCompatActivity implements IRecyclerActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Recyle-view the events and information
    ...
    private ArrayList<ParentObject> generateElements() {
        ...
        while(person_index.hasNext()) {
            ...
            if (add) {
                if (current.getGender().equals("Male")) family.addChild(new PersonElement(current.getFullName(), "SON", new IconDrawable(this, Iconify.IconValue.fa_male).colorRes(R.color.MaleIcon), current.getID()));
                else if (current.getGender().equals("Female")) family.addChild(new PersonElement(current.getFullName(), "DAUGHTER", new IconDrawable(this, Iconify.IconValue.fa_female).colorRes(R.color.FemaleIcon), current.getID()));
    ...
    public List<Object> getFamily(String person_id){ // Used for testing. basically same method as below but without drawables (which aren't accessable within tests)
        ...
        while(person_index.hasNext()) {
            ...
            if (current.hasFather() && current.getFather().equals(selected_person.getID())) add = true;
            if (add) {
                if (current.getGender().equals("Male")) family.addChild(new PersonElement(current.getFullName(), "SON", null, current.getID()));
                else if (current.getGender().equals("Female")) family.addChild(new PersonElement(current.getFullName(), "DAUGHTER", null, current.getID()));
                else family.addChild(new PersonElement(current.getFullName(), "CHILD", null, current.getID()));
            }
        }
